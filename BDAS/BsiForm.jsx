...
export default function BsiForm() {
  ...
  return (
    <div className="rounded-2xl p-5 border border-brand-border shadow-xl bg-gradient-to-b from-slate-900/70 to-slate-950/70">
      <div className="text-2xl font-extrabold text-brand-primary mb-1">New Defect Entry</div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        {/* Follow-up section */}
        <div className="md:col-span-2 rounded-2xl border border-brand-border bg-slate-900/40 p-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div
              ...
              className={`relative w-16 h-8 rounded-full cursor-pointer transition-colors ${
                form.tstTafEnabled ? "bg-brand-secondary" : "bg-slate-600"
              }`}
            >
              <div
          ...
          {form.defectImages?.length ? (
            <div className="mt-2 flex flex-wrap gap-2">
              {form.defectImages.slice(0, 6).map((f, i) => (
                <span key={i} className="px-2 py-1 rounded-lg bg-slate-800 border border-brand-border text-xs">
                  {f.name}
                </span>
              ))}
      ...
      <div className="mt-6 flex items-center justify-end gap-2">
        ...
        <button onClick={submit} disabled={busy} className="px-5 py-2 rounded-xl bg-emerald-600 hover:bg-brand-secondary text-black font-extrabold disabled:opacity-60">
          {busy ? "Saving..." : "Submit"}
        </button>
      </div>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="w-full max-w-md rounded-2xl p-4 border border-brand-border shadow-xl bg-gradient-to-b from-slate-900 to-slate-950">
            ...
            <div className="flex gap-2 justify-end mt-4">
              <button onClick={() => setOverrideOpen(false)} className="px-4 py-2 rounded-xl bg-slate-800 text-white border border-brand-border hover:border-sky-400/50" disabled={overrideBusy}>
                Cancel
              </button>
